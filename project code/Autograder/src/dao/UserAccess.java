...
//			stmt.setString(3, "Submitted");
			...
			while(rs.next())
				{
				ViewGrade ob = new ViewGrade(rs.getInt("assignmentId"),rs.getString("ques"),rs.getString("status") ,"-",rs.getDate("dateOfSub"));
				list.add(ob);
				}
			stmt.close();
			}
...
//				if(result > 0)
	...
	public String getFeedback(int assignmentID, String uname)
	{
		...
		try {
				stmt = myconn.prepareStatement("SELECT feedback from Student where assignmentId=? AND uname=?");
				stmt.setInt(1,assignmentID);
